        ...
        await webp2mp4File(savedFilename).then(async (rest) => {
            await Axios({ method: "GET", url: rest.result, responseType: "stream"}).then(({ data }) => {
                ...
                saving.on("finish", async () => {
                    await message.client.sendMessage(mid, fs.readFileSync('/root/WhatsAbu/stweb.mp4'), MessageType.video, { mimetype: Mimetype.mp4, caption: 'Made by LizaMwol', quoted: message.data })
   ...
   Abu.addCommand({pattern: 'mp4$', desc: Lang.ANİM_STİCK, fromMe: false}, (async (message, match) => {
        ...
        await webp2mp4File(savedFilename).then(async (rest) => {
            await Axios({ method: "GET", url: rest.result, responseType: "stream"}).then(({ data }) => {
                const saving = data.pipe(fs.createWriteStream('/root/WhatsAbu/stweb.mp4'))
                saving.on("finish", async () => {
                    await message.client.sendMessage(mid, fs.readFileSync('/root/WhatsAbu/stweb.mp4'), MessageType.video, { mimetype: Mimetype.mp4, caption: '𝙼𝚊𝚍𝚎 𝙱𝚢 𝙰𝚋𝚞 𝚜𝚎𝚛', quoted: message.data })
                    if (fs.existsSync(savedFilename)) fs.unlinkSync(savedFilename)
                    if (fs.existsSync('/root/WhatsAbu/stweb.mp4')) fs.unlinkSync('/root/WhatsAbu/stweb.mp4')
                })
            })
