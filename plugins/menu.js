...
if (Config.WORKTYPE == 'private') {
    ...
    Abu.addCommand({pattern: 'alive', fromMe: true, desc: Lang.ALIVE_DESC}, (async (message, match) => {
        
        let pp
        try { pp = await message.client.getProfilePicture(message.jid.includes('-') ? message.data.participant : message.jid ); } catch { pp = await message.client.getProfilePicture(); }
        await axios.get(pp, {responseType: 'arraybuffer'}).then(async (res) => { await message.client.sendMessage(message.jid, res.data, MessageType.image, { caption: Config.MENUMSG }); });
    }));

    Abu.addCommand({pattern: 'sysd', fromMe: true, desc: Lang.SYSD_DESC}, (async (message, match) => {
   ...
   Abu.addCommand({pattern: 'menu', fromMe: true, desc: Lang.ALIVE_DESC}, (async (message, match) => {
...
var i = Math.floor(31*Math.random())
    ...
    if (Config.MENUMSG.includes('{pp}')) {
        
        let pp
        try { pp = await message.client.getProfilePicture(message.jid.includes('-') ? message.data.participant : message.jid ); } catch { pp = await message.client.getProfilePicture(); }
        await axios.get(pp, {responseType: 'arraybuffer'}).then(async (res) => { await message.client.sendMessage(message.jid, res.data, MessageType.image, { caption: Config.MENUMSG.replace('{pp}', '').replace('{qt}', r_text[i])}); });
	}    
	   else {
	 ...
	 var p = Math.floor(1*Math.random())
        ...
        await message.client.sendMessage(message.jid, Buffer(plk_alive.data), MessageType.image, {mimetype: Mimetype.png, caption: Config.MENUMSG.replace('{qt}', r_text[i])})
	   }
    }));
    ...
    Abu.addCommand({pattern: 'psysd', fromMe: true, desc: Lang.SYSD_DESC, dontAddCommandList: true }, (async (message, match) => {
