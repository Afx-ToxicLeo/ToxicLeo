...
if (m.reply_message.sticker){
    return await m.client.sendMessage(m.jid,{text:"_"+(await webpUpload(await message.reply_message.downloadMediaMessage()))+"_"},{quoted: m.quoted})
}
    ...
    var q = await message.reply_message.downloadMediaMessage();
    await m.reply("_Converting to video & uploading..._")
...
try { await m.client.sendMessage(m.jid,{text:"_"+(await upload(await message.reply_message.downloadMediaMessage())).link+"_"},{quoted: m.quoted}) } catch {return await m.client.sendMessage(m.jid,{text:"_Failed to upload file!_"},{quoted: m.quoted});}
}
else return await m.reply("_Reply to image|video|audio|sticker_");
});
