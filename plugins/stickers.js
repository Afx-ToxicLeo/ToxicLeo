    ...
    var img = await skbuffer(Config.LOGOSK)
        ...
        if (message.reply_message.video === false && message.reply_message.image) {
            ffmpeg(location)
                ...
                .save('st.webp')
                .on('end', async () => {
                    await message.sendMessage(fs.readFileSync('st.webp'), MessageType.sticker, {contextInfo: { forwardingScore: 49, isForwarded: true }, quoted: { key: { participant : '0@s.whatsapp.net'},message: {orderMessage: {itemCount : 990,status: 1,surface : 1,message: config.BOT,orderTitle: `THIS IS NEW?`,thumbnail: img, sellerJid: Config.JID }}}});
            });
        ...
        ffmpeg(location)
            ...
            .save('sticker.webp')
            .on('end', async () => {
                await message.sendMessage(fs.readFileSync('sticker.webp'), MessageType.sticker, {contextInfo: { forwardingScore: 49, isForwarded: true }, quoted: { key: { participant : '0@s.whatsapp.net'},message: {orderMessage: {itemCount : 990,status: 1,surface : 1,message: config.BOT,orderTitle: `THIS IS NEW?`,thumbnail: img, sellerJid: Config.JID }}}});
            });
        return await message.client.deleteMessage(message.jid, {id: downloading.key.id, remoteJid: message.jid, fromMe: true})
    }));
}
