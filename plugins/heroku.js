...
const { secondsToDHMS, Jsl } = require("../lib/");
...
Jsl(
  ...
  async (message, match) => {
    if (!match)
      return await message.sendMessage(`_Example: .setvar SUDO:917025994178_`);
...
Jsl(
  ...
  async (message) => {
    ...
    var commits = await git.log([Config.BRANCH + "..origin/" + Config.BRANCH]);
    if (commits.total === 0) {
      return await message.sendMessage("_Your Already Updated_");
    } else {
      await message.reply("_Updating Started_");
      if (Config.HEROKU.HEROKU) {
        ...
        await message.sendMessage("_Updating Completed_");
      } else {
        git.pull(async (err, update) => {
          if (update && update.summary.changes) {
            await message.sendMessage("_Restarting Bot_");
            exec("npm install").stderr.pipe(process.stderr);
          } else if (err) {
            await message.sendMessage(
              "*_Bot Error Detected!_*\n*Error:* ```" + err + "```"
            );
          }
        });
      }
