...
Abu.addCommand({pattern: 'plugin ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC, warn: Lang.WARN, dontAddCommandList: true}, (async (message, match) => {

    if (match[1] == '') return await message.client.sendMessage(message.jid,Lang.NEED_URL + '.install https://gist.github.com/Afx-Abu/4232b1c8c4734e1f06c3d991149c6fbd', MessageType.text)
    try {
    ...
    if (response.statusCode == 200) {
        ...
        try {
            ...
            fs.unlinkSync('/root/WhatsAbu/plugins/' + plugin_name + '.js')
...
Abu.addCommand({pattern: 'plugin list$', fromMe: true, dontAddCommandList: true, desc: Lang.PLUGIN_DESC}, (async (message, match) => {
    ...
    if (plugins.length < 1) {
        ...
        plugins.map(
            (plugin) => {
                let vf = plugin.dataValues.url.includes('Afx-Abu') ? msg : unmsg
...
Abu.addCommand({pattern: 'remove plugin(?: |$)(.*)', fromMe: true, dontAddCommandList: true, desc: Lang.REMOVE_DESC}, (async (message, match) => {
